request = use('App/Lib/LMS/TalentLms/lib/request')
class Courses {
     ...
     * @param obj.role The role of the user in the course.
     */
    addUserToCourse(obj) {
        if (!obj.user_id || !obj.course_id || !obj.role) {
            console.log(obj)
            throw new Error('For adding course object has to have user_id and course_id and role');
        }
     ...
     * @param userId The id of the user.
     */
    goToCourse(courseId, userId, logoutUrl) {
      if(!logoutUrl){
        return this._request('GET', `gotocourse/user_id:${userId},course_id:${courseId}`);
      }
        return this._request('GET', `gotocourse/user_id:${userId},course_id:${courseId},logout_redirect:${Buffer.from(logoutUrl).toString('base64')}`);
    }

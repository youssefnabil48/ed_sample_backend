...
class Users {
    ...
    setStatus(userId, status) {
        return this._request('GET', `usersetstatus/user_id:${userId},status:${status}`);
    }
   ...
   deleteUser(obj) {
    if (!obj.user_id) {
        throw new Error('To delete a user user_id is required');
    }
    ...
    * @param obj.timezone User timezone
     ...
     */
    userLogin(obj, branchName = null) {
        if (!obj.login || !obj.password) {
            throw new Error('For Logging a user login and password are required');
        }
        if(branchName){
          return this._request('POST', `userlogin`, obj, branchName+'-' + this.hostname);
        }
